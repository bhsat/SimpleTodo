...
public class TodoDbHelper extends SQLiteOpenHelper {
    ...
    private static final String KEY_TODO_POS = "todoPosition";
    ...
    public long addTodoItem(Todo item) {
        ...
        try {
            ...
            values.put(KEY_TODO_PRIORITY, item.getTodoPriority());
            values.put(KEY_TODO_POS, item.getTodoPosition());
    ...
    public int updateItem(Todo item) {
        ...
        try {
            ...
            values.put(KEY_TODO_ITEM, item.getTodoItem());
            values.put(KEY_TODO_PRIORITY, item.getTodoPriority());
            rows = db.update(TABLE_TODO, values, KEY_TODO_POS + " = ?", new String[]{String.valueOf(item.getTodoPosition())});
            db.setTransactionSuccessful();
        } catch(Exception e) {
    ...
    public void deleteItem(int position) {
        ...
        db.beginTransaction();
        try {
            db.delete(TABLE_TODO, KEY_TODO_POS + " = ?", new String[] {String.valueOf(position)});
            db.setTransactionSuccessful();
        } catch(Exception e) {
        ...
        try {
            if (cursor.moveToFirst()) {
                do {
                    ...
                    item.setTodoPriority(cursor.getString(cursor.getColumnIndex(KEY_TODO_PRIORITY)));
                    item.setTodoPosition(cursor.getInt(cursor.getColumnIndex(KEY_TODO_POS)));
                    todoList.add(item);
